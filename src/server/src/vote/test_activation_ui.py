...
class SleepingPlayerCardTests(TestCase):
    ...
    def test_active_player_keeps_the_normal_vote_and_layer_controls(self):
        ...
        self.assertContains(response, "data-submit-vote")
        self.assertContains(response, "SUBMIT VOTE")
        self.assertEqual(NFCActionParser(html).cards, 2)
        self.assertEqual(NFCActionParser(html).prompt_cards, 1)
        self.assertContains(response, 'aria-label="Pleasant vote: 0 downvote, 1 upvote"')
        ...
        self.assertEqual(len(actions), 1)
        self.assertEqual(actions[0][":hx-vals"], "submissionPayload")
        self.assertIn("data-submit-vote", actions[0])
